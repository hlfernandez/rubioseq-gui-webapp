 ...
 *
 */
public class ConfigProgramPathsCNVModel {

	private ConfigProgramPathsCNV config = new ConfigProgramPathsCNV();
	...
	public void init() {
		File cppFile = (File) Sessions.getCurrent().getAttribute(
				ConfigurationViewModel.CPP_CNV);
		if (cppFile != null) {
		...
		Sessions.getCurrent().removeAttribute(ConfigurationViewModel.CPP_CNV);
	}
	...
	public ConfigProgramPathsCNV getConfiguration() {
		return config;
	}
	
	public void setConfiguration(ConfigProgramPathsCNV cfg) {
		this.config = cfg;
	}
	...
	@DependsOn({ "configuration.bwaPath", "configuration.samtoolsPath",
			...
			"configuration.bedtoolsPath", "configuration.contraPath",
